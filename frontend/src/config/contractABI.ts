...
export const BILL_SPLITTER_ABI = [
  // Events
  "event BillCreated(uint256 indexed billId, address indexed creator, address indexed recipient, string name, uint256 totalAmount, uint8 splitType, uint256 deadline, uint256 participantCount)",
  ...
  // Functions
  "function createBill(string memory _name, uint256 _totalAmount, address _recipient, address[] memory _participants, uint256[] memory _amounts, uint256 _deadline) external returns (uint256)",
  "function payShare(uint256 _billId) external",
  "function getBill(uint256 _billId) external view returns (tuple(uint256 billId, address creator, address recipient, string name, uint256 totalAmount, uint8 splitType, uint256 deadline, uint256 createdAt, uint8 status, uint256 settledAt, tuple(address wallet, uint256 amount, bool hasPaid, uint256 paidAt)[] participants))",
